...
class PolymarketDataToolkit(BaseToolkit):
    ...
    def scan_markets_by_category(
        ...
        async def _scan():
            try:
                markets = await self.client.filter_markets_by_category(
                    category=category, 
                    limit=limit
                )
    ...
    def get_tools(self) -> List["FunctionToolType"]:
        ...
        scan_markets_tool.__doc__ = "Scan Polymarket markets by category with liquidity filters"
        tools.append(FunctionTool(scan_markets_tool))
        ...
        search_conviction_tool.__doc__ = "Search high-conviction markets matching query"
        tools.append(FunctionTool(search_conviction_tool))
        ...
        tools.append(FunctionTool(market_data_tool))
        ...
        def position_sizing_tool(
            ...
            """Calculate position sizes based on wallet and risk limits"""
        ...
        position_sizing_tool.__doc__ = "Calculate position sizes based on wallet and risk limits"
        tools.append(FunctionTool(position_sizing_tool))
        
        # Order planning tool
        def order_planning_tool(orders: str) -> Dict[str, Any]:
            """Plan batch of orders ready for execution"""
            # orders parameter is a JSON string to avoid Pydantic forward-ref issues
            import json
            if isinstance(orders, str):
                orders_list = json.loads(orders)
            else:
                orders_list = orders
            return toolkit.plan_order_batch(orders=orders_list)
        
        order_planning_tool.__doc__ = "Plan batch of orders ready for execution"
        tools.append(FunctionTool(order_planning_tool))
        ...
        monitor_pos_tool.__doc__ = "Monitor current open positions and P&L"
        tools.append(FunctionTool(monitor_pos_tool))
        ...
        position_history_tool.__doc__ = "Get historical/closed positions (raw trade history)"
        tools.append(FunctionTool(position_history_tool))
